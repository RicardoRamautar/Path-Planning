...
from environment.variables import RAD_OBST, RAD_NP, RAD_NBR, RRT_ITERATIONS
...
######### PLOT FUNCTION #########
def plotTrees(tree, path, spheres, start_point, end_point):
    plt.figure()
    ax = plt.gca()
...
class RRT_star():
    def __init__(self, START_POINT, END_POINT, BOUNDS, SPHERES):
        ...
        self.tree = {START_POINT: Vertex(START_POINT)}
        self.START_POINT = START_POINT
        self.END_POINT = END_POINT

    def findNearestNode(self, random_point):
        ...
        for node_pos, node in self.tree.items():
    ...
    def addNewPoint(self, new_point):
        ...
        self.tree[tuple(new_point)] = new_node

        return neighbours, new_node

    def updateNeighbours(self, neighbours, new_node):
        for nbr_node, edge_length in neighbours.items():
            ...
            if (new_node.min_path_length + edge_length) < nbr_node.min_path_length:
                self.tree[nbr_position].predecessor = new_node
                self.tree[nbr_position].min_path_length = new_node.min_path_length + edge_length
    ...
    @property
    def path(self):
        '''Finds shortest path of the tree'''
        min_dist = np.Inf
        nearest_node = None

        for node_pos, node in self.tree.items():
            dist = euclideanDistance(self.END_POINT, node_pos)
            if dist < min_dist:
                min_dist = dist
                nearest_node = node
        ...
        # Checks if connection is possible between end point and the nearest node to the end point
        if not (nearest_node.getTupleCoordinates() == self.END_POINT):
            assert self.collisionCheck(np.array(list(self.END_POINT)), nearest_node.getNpCoordinates()), "NO PATH FROM START TO END  COULD BE FOUND!"
            min_path_length = nearest_node.min_path_length + euclideanDistance(self.END_POINT, nearest_node.getTupleCoordinates())
            new_node = Vertex(self.END_POINT, nearest_node, min_path_length)
            self.tree[self.END_POINT] = Vertex(self.END_POINT, nearest_node, min_path_length)
            return self.returnPartialPath(new_node, self.START_POINT)
        else:
            return self.returnPartialPath(nearest_node, self.START_POINT)
    ...
    def solve(self, N = RRT_ITERATIONS):
        ...
        for i in range(N):
            ...
            if i%25 == 0: x,y,z = self.END_POINT # add "and solution not found" ?
            ...
            nearest_node = self.findNearestNode(random_point)
            ...
            neighbours, new_node = self.addNewPoint(new_point)
                    
            if not neighbours:
                continue

            self.updateNeighbours(neighbours, new_node)

        plotTrees(self.tree, [], self.SPHERES, self.START_POINT, self.END_POINT)
